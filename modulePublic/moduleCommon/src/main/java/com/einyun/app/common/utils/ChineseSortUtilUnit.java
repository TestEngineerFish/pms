...
//
     ...
     * @param list
     */
    private static void exchangeNameOrderHouse(int j, List<BuildModel.GridRangeBean> list) {
        String namePinYin1 = Pinyin.toPinyin(list.get(j).getCode().replaceAll("-",""),"");
        String namePinYin2 = Pinyin.toPinyin(list.get(j + 1).getCode().replaceAll("-",""),"");
        ...
        for (int i = 0; i < size; i++) {
            ...
            if (jc > jcNext) {//A在B之后就直接交换,让A在前面B在后面
                BuildModel.GridRangeBean nameBean = list.get(j);
     ...
     */
    @RequiresApi(api = Build.VERSION_CODES.N)
    public static void transferListBuild(List<BuildModel.GridRangeBean> list) {
        for (int i = 0; i < list.size() - 1; i++) {
            for (int j = 0; j < list.size() - 1 - i; j++) {
     ...
     */
    @RequiresApi(api = Build.VERSION_CODES.N)
    public static void transferListBuildDown(List<BuildModel.GridRangeBean> list) {
        for (int i = 0; i < list.size() - 1; i++) {
            for (int j = 0; j < list.size() - 1 - i; j++) {
     ...
     * @param list
     */
    private static void exchangeNameOrderBuildDown(int j, List<BuildModel.GridRangeBean> list) {
        String code1 = list.get(j).getCode();
        if (code1.length()==1) {
            code1="0"+code1;
        }
        String code2 = list.get(j+1).getCode();
        if (code2.length()==1) {
            code2="0"+code2;
        }
     ...
     * @param list
     */
    private static void exchangeNameOrderBuild(int j, List<BuildModel.GridRangeBean> list) {
        String code1 = list.get(j).getCode();
        if (code1.length()==1) {
            code1="0"+code1;
        }
