...
public class PeriodizationNoAutoJumpView extends DialogFragment implements ItemClickListener<OrgModel>, View.OnClickListener {
    ...
    private void loadData(List<OrgModel> orgModels) {
        if (adapter == null) {
            adapter = new RVBindingAdapter<ItemBlockChooseNoJumpBinding, OrgModel>(getActivity(), com.einyun.app.common.BR.org) {
                @Override
                public void onBindItem(ItemBlockChooseNoJumpBinding itemBinding, OrgModel model, int pos) {
                    if (model.getGrade().equals(DataConstants.KEY_ORG_DIVIDE)) {
                        itemBinding.ivRightselect.setVisibility(View.GONE);
                    }
                    itemBinding.ivRightselect.setOnClickListener(new View.OnClickListener() {
                        @Override
                        public void onClick(View v) {
