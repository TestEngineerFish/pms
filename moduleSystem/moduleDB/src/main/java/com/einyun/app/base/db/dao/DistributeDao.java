...
public interface DistributeDao {
    ...
    @Query("select * from distributes where userId=:userId and orderType=:orderType ORDER BY createTime DESC")
    DataSource.Factory<Integer, Distribute> queryAll(String userId,int orderType);


    @Query("select * from distributes where userId=:userId and orderType=:orderType ORDER BY createTime DESC LIMIT :current,:pageSize ")
