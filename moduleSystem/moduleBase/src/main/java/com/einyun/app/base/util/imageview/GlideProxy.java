 ...
 * @ClassName: GlideProxy
 * @Description: java类作用描述
 * @Author: chumingjun
 * @CreateDate: 2019/09/26 18:19
 * @UpdateUser: 更新者：
 * @UpdateDate: 2019/09/26 18:19
 * @UpdateRemark: 更新说明：
 * @Version: 1.0
 */
public class GlideProxy implements IProx{
    /**
     * Glide特点
     * 使用简单
     * 可配置度高，自适应程度高
    ...
    public  void loadImageViewAnim(Context mContext, String path, int anim, ImageView mImageView) {
        Glide.with(mContext).load(path).transition(GenericTransitionOptions.with(anim)).into(mImageView);
    }
    ...
    //设置监听请求接口
    public  void loadImageViewListener(Context mContext, String path, ImageView mImageView, RequestListener<Drawable> requstlistener) {
    ...
    //设置要加载的内容
    public  void loadImageViewContent(Context mContext, String path, SimpleTarget<Drawable> simpleTarget) {
